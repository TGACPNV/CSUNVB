            ...
            <?php foreach ($batchesByDrugId[$drug["id"]] as $batch): ?>
                ...
                <tr>
                    ...
                    <?php foreach ($novas as $nova): ?>
                        ...
                        <?php $ncheck = getNovaCheckByDateAndDrug($date, $drug['id'], $nova['id'], $drugsheet['id']); // not great practice, but it spares repeated queries on the db ?>
                        <td id="<?= $UID ?>">
                            <input type="number" min="0" class="text-center" value="<?= $ncheck ? $ncheck["start"] : ''?>" onchange="cellUpdate(<?= "'" . $UID . "'" ?>);" id="<?= $UID ?>start">
                            <input type="number" min="0" class="text-center" value="<?= $ncheck ? $ncheck["end"] : '' ?>" onchange="cellUpdate(<?= "'" . $UID . "'" ?>);" id="<?= $UID ?>end">
                        </td>
                    <?php endforeach; ?>
                ...
                <?php foreach ($batchesByDrugId[$drug["id"]] as $batch): ?>
                    ...
                    <tr>
                        <td class="text-right"><?= $batch['number'] ?></td>
                        <td class="text-center">
                            <input id="<?= $UID ?>start" type="number" min="0" value="<?= $pcheck ? $pcheck['start'] : '' ?>" onchange="cellUpdate(<?= "'" . $UID . "'"?>);">
                        </td>
                        <?php foreach ($novas as $nova): ?>
                            <td class="text-center">
                                <input class="<?= $UID ?> nova" type="number" min="0" value="<?= getRestockByDateAndDrug($date,$batch['id'],$nova['id']) ?>" onchange="cellUpdate(<?= "'" . $UID . "'"?>);">
                            </td>
                        <?php endforeach; ?>
                        <td id="<?= $UID ?>" class="text-center">
                            <input id="<?= $UID ?>end" type="number" min="0" value="<?= $pcheck ? $pcheck['end'] : '' ?>" onchange="cellUpdate(<?= "'" . $UID . "'"?>);">
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php endforeach; ?>
